...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  // ── Called by VoiceScreen when user says "take me to Notes" etc. ─────────
  void _handleVoiceNavigation(int tabIndex) {
  ...
  Future<void> _initVosk() async {
    if (kIsWeb) return;
    try {
      debugPrint('=== VOSK HOME: Skipping on this platform ===');
    } catch (e) {
  ...
  void _openNova() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _NovaSheet(onNavigate: _handleVoiceNavigation),
    ).then((_) {
...
// ── _NovaSheet now accepts onNavigate callback ────────────────────────────────
class _NovaSheet extends StatefulWidget {
  final void Function(int tabIndex) onNavigate;
  const _NovaSheet({required this.onNavigate});
  @override
          ...
          child: Column(
            children: [
              Center(child: Container(
                ...
                width: 36, height: 4,
                decoration: BoxDecoration(color: VapaColors.border, borderRadius: BorderRadius.circular(2)),
              )),
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Text(_greeting, style: const TextStyle(color: VapaColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w600)),
                        const Text('Nova — Voice Assistant', style: TextStyle(color: VapaColors.tealLight, fontSize: 12)),
                      ],
                    ),
                    const Spacer(),
                    IconButton(
                      icon: const Icon(Icons.close, color: VapaColors.textMuted),
              ...
              // ── VoiceScreen now receives the navigation callback ──────────
              Expanded(
                child: VoiceScreen(
                  onNavigate: (tabIndex) {
...
class _ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Profile'), backgroundColor: VapaColors.bg),
      body: ListView(
        ...
        children: [
          Center(child: Column(children: [
            Container(
              ...
              child: Center(child: Text(initials, style: const TextStyle(color: VapaColors.textPrimary, fontSize: 28, fontWeight: FontWeight.w600))),
            ),
            const SizedBox(height: 12),
            Text(email, style: const TextStyle(color: VapaColors.textSecondary, fontSize: 14)),
          ])),
          const SizedBox(height: 32),
          _StatsRow(),
          const SizedBox(height: 32),
          const _SectionLabel('Settings'),
          _SettingsTile(icon: Icons.auto_awesome, label: 'AI Model', value: 'Groq — Llama 3.3 70B', onTap: () {}),
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(children: [
          Text(value, style: const TextStyle(color: VapaColors.tealLight, fontSize: 24, fontWeight: FontWeight.w600)),
          const SizedBox(height: 4),
          Text(label, style: const TextStyle(color: VapaColors.textMuted, fontSize: 12)),
        ]),
      ),
    );
  }
...
class _SettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListTile(
        onTap: onTap,
        leading: Icon(icon, color: VapaColors.textSecondary, size: 20),
        title: Text(label, style: const TextStyle(color: VapaColors.textPrimary, fontSize: 14)),
        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
          Text(value, style: const TextStyle(color: VapaColors.textMuted, fontSize: 13)),
          const SizedBox(width: 4),
          const Icon(Icons.chevron_right, color: VapaColors.textMuted, size: 18),
        ]),
      ),
    );
  }
